...
app.get('/', function(req, res) {
    if(currentUser){
    	res.render('index', { title: 'Pictroid', username:cachedCurrentUser.get("username"), authed:true});
    } else {
    	res.render('index');
    }
});
app.get('/about', function(req, res) {
    if(currentUser){
    	res.render('about', { username:cachedCurrentUser.get("username"), authed:true});
    } else {
    	res.render('about');
    }
});
...
app.get('/explore/:filter?', function(req, res) {
	if(currentUser){
    	res.render('explore', { filter: req.params.filter, username:cachedCurrentUser.get("username"), authed:true});
    } else {
...
app.get('/pic/:id', function(req, res) {
	if(currentUser){
    	db.asteroids.query.getPic(req.params.id).then(function(result) {
			res.render('details', { picObject: result, username:cachedCurrentUser.get("username"), authed:true});
		}, function() { 
...
app.get('/upload', function(req, res) {
	// Code to authedorize
	if (currentUser) {
		res.render('user/upload', { username:cachedCurrentUser.get("username"), authed:true});
	} else {
...
app.get('/user/:name', function(req, res) {
	if (currentUser) {
		res.render('user/profile', { username: req.params.name, username:cachedCurrentUser.get("username"), authed:true});
	} else {
...
app.get('/account/settings', function(req, res) {
	// Code to authedorize
	if (currentUser) {
		res.render('account/settings', { username:cachedCurrentUser.get("username"), email:cachedCurrentUser.get("email"), status:cachedCurrentUser.get("status"), profileImgSrc:cachedCurrentUser.get("profileImg"), authed:true, secure:true});
	} else {
		res.redirect('/signin');
	}
});
...
app.post('/signin', function(req, res) {
	Parse.User.logIn(req.body.username.toLowerCase(), req.body.password, {
	  success: function(user) {
	    ...
	    cachedCurrentUser.set("username", currentUser.attributes.username, function(err, reply) {
