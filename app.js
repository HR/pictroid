...
app.get('/about', function(req, res) {
    if(currentUser){
    	res.render('about', { username:currentUser.attributes.username, authed:true});
    } else {
    	res.render('about');
    }
});
...
app.get('/explore/:filter?', function(req, res) {
	if(currentUser){
    	res.render('explore', { filter: req.params.filter, username:currentUser.attributes.username, authed:true});
    } else {
...
app.get('/user/:name', function(req, res) {
	if (currentUser) {
		res.render('user/profile', { username: req.params.name, username:currentUser.attributes.username, authed:true});
	} else {
...
app.get('/account/settings', function(req, res) {
	// Code to authedorize
	if (currentUser) {
		res.render('account/settings', { username:currentUser.attributes.username, email:currentUser.attributes.email, status:currentUser.attributes.status, profileImgSrc:currentUser.get("profileImg").url(), autheded:true, secure:true});
	} else {
		res.redirect('/signin');
	}
});
